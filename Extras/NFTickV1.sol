...
/*
create an array to store all tokenIds
update tokenId array whenever an event is created 
use a loop of tokenId array to get the balance of each users nfticks 
for fetchallmyNfticks function call the getBatchBalance function in it 
*/
contract NFTick is ERC1155, ERC1155Supply, ERC1155Holder{
    ...
    uint256 listingPrice = 0.1 ether; // ether is actually because contrcat will be deployed on matic

    // an array to store all tokenIds
    ...
    function fetchAllMyNfticks() public view returns(nftick[] memory) {
        ...
        // uint256[] memory userBalance = new uint256[](allTokenIds.length);

        /*
        create a loop to loop through all elements in allTokenIds array, 
        use tokenIds to determine balance of user nfticks 
        */
        // for(uint256 i = 0; i < allTokenIds.length; i++) {
        // uint256 id = allTokenIds[i]; 
        // userBalance[i] = getBalance( msg.sender , id);
        // }

        nftick[] memory userNfticks = new nftick[](totalNoOfNfticks);
        for(uint256 i = 0; i < totalNoOfNfticks; i++ ){
        if(AllNfticksCreated[i].owner == msg.sender){
            uint currentId = i;
            // reference current collection
            nftick storage currentNftick = AllNfticksCreated[currentId];
            // store collection in the array
            userNfticks[currentIndex] = currentNftick;
            // increment item index
            currentIndex += 1;
        }
        }
